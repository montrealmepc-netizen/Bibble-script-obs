...
# ----------------------------------------------------------------------
# --- OBS Integration Functions (Inchangé) ---
# ----------------------------------------------------------------------
def update_obs_text():
...
def set_scene_item_visibility(source_name, visible):
    ...
    current_scene = obs.obs_scene_from_source(current_scene_source); scene_item = obs.obs_scene_find_source_recursive(current_scene, source_name)
    if scene_item: obs.obs_sceneitem_set_visible(scene_item, visible); 
...
class BibleNavigator(ctk.CTk):
    ...
    def _create_sidebar(self):
        sidebar_frame = ctk.CTkFrame(self, width=200, corner_radius=0); sidebar_frame.grid(row=0, column=0, sticky="nsw"); sidebar_frame.grid_rowconfigure(3, weight=1) 
        ctk.CTkLabel(sidebar_frame, text="Navigation", font=ctk.CTkFont(size=20, weight="bold")).grid(row=0, column=0, padx=20, pady=20)
        self.bible_combo = ctk.CTkComboBox(sidebar_frame, values=[], command=self.on_bible_selected); self.bible_combo.grid(row=1, column=0, padx=20, pady=(0,10), sticky="ew") 
        nav_frame = ctk.CTkFrame(sidebar_frame, fg_color="transparent"); nav_frame.grid(row=2, column=0, padx=20, pady=10, sticky="ew"); nav_frame.columnconfigure((0,1), weight=1) 
        ...
        self.book_list_frame = ctk.CTkScrollableFrame(sidebar_frame, label_text="Livre"); self.book_list_frame.grid(row=3, column=0, padx=20, pady=10, sticky="nsew"); self.book_buttons = {} 
    ...
    # --- MÉTHODE ENTIÈREMENT MODIFIÉE pour la nouvelle disposition ---
    def _create_editor_tab(self):
        editor_tab = self.tab_view.tab("Éditeur")
        
        # --- Configurer la grille de l'onglet ---
        # Colonne 0 (flexible) pour les éditeurs
        editor_tab.grid_columnconfigure(0, weight=1) 
        # Colonne 1 (fixe) pour les prévisualisateurs
        editor_tab.grid_columnconfigure(1, weight=0) 
        
        # Poids des lignes pour la colonne 0 (Éditeurs)
        editor_tab.grid_rowconfigure(0, weight=3) # Éditeur principal
        editor_tab.grid_rowconfigure(1, weight=0) # Boutons de contrôle
        editor_tab.grid_rowconfigure(2, weight=2) # Éditeur verset suivant
        editor_tab.grid_rowconfigure(3, weight=0) # Label de statut
        
        # --- Colonne 0: Éditeurs et Contrôles ---
        
        # --- Éditeur Principal (row 0, col 0) ---
        editor_frame = ctk.CTkFrame(editor_tab, fg_color=("gray90", "gray20"))
        ...
        self.editor_text = tk.Text(editor_frame, wrap=tk.WORD, relief="flat", borderwidth=0, font=("Segoe UI", 14), undo=True, height=10); 
        self.editor_text.pack(fill="both", expand=True, padx=5, pady=5)
        
        # --- Contrôles (row 1, col 0) ---
        control_frame = ctk.CTkFrame(editor_tab, fg_color="transparent")
        ...
        # --- Éditeur Verset Suivant (row 2, col 0) ---
        preview_frame = ctk.CTkFrame(editor_tab)
        preview_frame.grid(row=2, column=0, sticky="nsew")
        preview_frame.grid_rowconfigure(1, weight=1); preview_frame.grid_columnconfigure(0, weight=1)
        ...
        # --- Statut (row 3, col 0) ---
        self.status_label = ctk.CTkLabel(editor_tab, text="", anchor="center")
        self.status_label.grid(row=3, column=0, sticky="ew", pady=(10, 0))

        # --- Colonne 1: Prévisualisateurs (Fixe) ---
        
        # Largeur fixe pour la colonne de droite
        preview_column_width = 350
        preview_text_wraplength = preview_column_width - 30 # Largeur - padding
        
        # Frame principale pour la colonne de droite
        preview_stack_frame = ctk.CTkFrame(editor_tab, width=preview_column_width)
        preview_stack_frame.grid(row=0, column=1, sticky="ns", rowspan=4, padx=(10, 0), pady=(0, 10))
        preview_stack_frame.grid_propagate(False) # Empêche le redimensionnement
        
        preview_stack_frame.grid_columnconfigure(0, weight=1)
        preview_stack_frame.grid_rowconfigure(1, weight=1) # Poids pour le 1er preview
        preview_stack_frame.grid_rowconfigure(3, weight=1) # Poids pour le 2e preview

        # --- Prévisualisation Actuelle ("Live") ---
        ctk.CTkLabel(preview_stack_frame, text="Actuel (Live)", font=self.bold_font).grid(row=0, column=0, sticky="w", padx=10, pady=(10,5))
        current_preview_frame = ctk.CTkFrame(preview_stack_frame, fg_color="black", corner_radius=5)
        current_preview_frame.grid(row=1, column=0, sticky="nsew", padx=10, pady=5)
        current_preview_frame.grid_propagate(False) 
        self.current_block_preview = ctk.CTkLabel(current_preview_frame, text="...", font=("Segoe UI", 16), text_color="white", justify="center", wraplength=preview_text_wraplength)
        self.current_block_preview.pack(expand=True, fill="both", padx=10, pady=10)
        # La ligne .bind(...) a été supprimée

        # --- Prévisualisation Suivante ("Cued") ---
        ctk.CTkLabel(preview_stack_frame, text="Suivant (Cued)", font=self.bold_font).grid(row=2, column=0, sticky="w", padx=10, pady=(10,5))
        next_preview_frame = ctk.CTkFrame(preview_stack_frame, fg_color="black", corner_radius=5)
        next_preview_frame.grid(row=3, column=0, sticky="nsew", padx=10, pady=10)
        next_preview_frame.grid_propagate(False) 
        self.next_block_preview = ctk.CTkLabel(next_preview_frame, text="...", font=("Segoe UI", 16), text_color="white", justify="center", wraplength=preview_text_wraplength)
        self.next_block_preview.pack(expand=True, fill="both", padx=10, pady=10)
        # La ligne .bind(...) a été supprimée
    ...
    # --- MÉTHODE SUPPRIMÉE ---
    # def _on_preview_resize(self, event): (La méthode a été supprimée)

    def _update_live_cued_previews(self):
        try:
            current_block_text = ""
            next_block_text = ""
            current_index = self.manager.current_block_index
            blocks = self.manager.display_blocks

            # 1. Obtenir le texte du bloc actuel (Live)
            if 0 <= current_index < len(blocks):
                current_block_text = blocks[current_index]

            # 2. Obtenir le texte du bloc suivant (Cued)
            next_index = current_index + 1
            if 0 <= next_index < len(blocks):
                next_block_text = blocks[next_index]
            else: 
                next_verse_full_text = self.preview_text.get("1.0", tk.END).strip()
                if next_verse_full_text:
                    next_block_text = next_verse_full_text.split('\n\n')[0]
                else:
                    ref, text = self.manager.get_next_verse_info()
                    if text:
                        formatted_next = self.manager._auto_format_verse_for_editor(text)
                        next_block_text = formatted_next.split('\n\n')[0]
                    else:
                        next_block_text = "..." 
            
            # 3. Mettre à jour les labels
            self.current_block_preview.configure(text=current_block_text or "...")
            self.next_block_preview.configure(text=next_block_text or "...")
        except AttributeError:
            pass
        except Exception as e:
            print(f"Erreur mise à jour prévisualisateurs: {e}")
    ...
    def on_editor_key_release(self, event=None):
        ...
        self._update_live_cued_previews() 
    ...
    def on_book_selected(self, book_info):
        for btn in self.book_buttons.values(): 
            btn.configure(fg_color="transparent", text_color="gold") 
            
        selected_button = self.book_buttons.get(book_info['name'])
        if selected_button:
            selected_button.configure(fg_color=("gray85", "gray20"), text_color=ctk.ThemeManager.theme["CTkButton"]["text_color"]) 
    ...
    def update_book_list(self):
        ...
        self.sync_nav_to_current_verse() 
    ...
    def update_display(self):
        ...
        self.update_next_verse_preview() 
    ...
    def sync_nav_to_current_verse(self):
        if self.manager.current_book_name and self.manager.current_chapter > 0 and self.manager.current_verse_index > 0:
            ...
            if current_button:
                   current_button.configure(fg_color=("gray85", "gray20"), text_color=ctk.ThemeManager.theme["CTkButton"]["text_color"]) 
...
# ----------------------------------------------------------------------
# --- OBS Script Boilerplate (Inchangé) ---
# ----------------------------------------------------------------------
